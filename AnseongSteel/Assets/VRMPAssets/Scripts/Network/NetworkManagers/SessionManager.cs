...
namespace XRMultiplayer
{
    ...
    public class SessionManager : MonoBehaviour
    {
        ...
        {
            m_Status = new BindableVariable<string>("");
        }
        ...
        /// </summary>
        /// <returns></returns>
        // QuerySessionsAsync can throw a SessionException wrapping a NullReferenceException if the
        // UGS session browser hasn't finished initializing after authentication completes. This is
        // a transient race in the SDK — retrying after a short delay is the correct fix.
        const int k_MaxQueryRetries = 3;

        async Task<QuerySessionsResults> QuerySessionsWithRetry()
        {
            // No exit condition in the loop header; termination is handled by the two paths inside:
            // either `return` on success, or the `when` guard on the catch stops matching once
            // attempt >= k_MaxQueryRetries - 1, letting the final SessionException propagate out.
            for (int attempt = 0; ; attempt++)
            {
                try
                {
                    return await MultiplayerService.Instance.QuerySessionsAsync(GetQuickJoinFilterOptions());
                }
                catch (SessionException) when (attempt < k_MaxQueryRetries - 1)
                {
                    Utils.Log($"{k_DebugPrepend}QuerySessionsAsync transient failure (attempt {attempt + 1}/{k_MaxQueryRetries}), retrying...", 1);
        ...
        public async Task<ISession> QuickJoinLobby()
        {
            ...
            try
            {
                QuerySessionsResults results = await QuerySessionsWithRetry();
        ...
        public async Task LeaveSession()
        {
            if (m_CurrentSession != null)
            {
                ...
                {
                    // Session services may already be disposed when stopping play mode — safe to ignore.
                }
