...
public static class PilotV03Builder
{
    public const string Root="Assets/_Project/02_Art/PilotV03";
    public const string ScenePath=Root+"/Scenes/PilotRigReview_v03.unity";
    [MenuItem("Anseong Steel/Pilot V03/Build model and rig review")]
    public static void Build()
    {
        ...
        string master=Root+"/AS_Pilot_v03.fbx", candidate=Root+"/AS_Pilot_v03_QuestCandidate.fbx";
        ...
        if(!report.pass)throw new Exception("Pilot V03 review validation failed. See unity_validation.json");
        EditorSceneManager.SaveScene(scene,ScenePath);AssetDatabase.SaveAssets();AssetDatabase.Refresh();
        AssetDatabase.ExportPackage(Root,Path.GetFullPath("PilotV03_Review.unitypackage"),ExportPackageOptions.Recurse);
        Debug.Log("PILOT_V03_REVIEW_PASS "+JsonUtility.ToJson(report));
    }
    ...
    static ModelReport Validate(PilotReviewRig rig,int mapped)
    {
        ...
        Vector3 axis=(rig.left.hand.position-rig.left.lower.position).normalized;
        Quaternion start=rig.left.forearmTwist.localRotation;
        rig.left.target.rotation=Quaternion.AngleAxis(72,axis)*rig.left.target.rotation;rig.Evaluate();
        report.forearmResponseDegrees=Quaternion.Angle(start,rig.left.forearmTwist.localRotation);
        ...
        report.pass=report.avatarValid&&mapped==52&&report.fingerBones==30&&report.helperBones==6&&report.renderers==2&&report.unweighted==0&&report.badWeightSums==0&&report.nonFiniteVertices==0
            &&report.poses.All(p=>p.pass)&&report.maxFootDrift<.0001f&&report.repeatDriftDegrees<.05f&&Mathf.Abs(report.forearmResponseDegrees-36)<.2f&&Mathf.Abs(report.headResponseDegrees-35)<.2f&&report.leftFingerTravel>.02f&&report.rightFingerDrift<.0001f;
        return report;
    }
    ...
    [Serializable] public class ModelReport
    {
        ...
        public float maxFootDrift,repeatDriftDegrees,forearmResponseDegrees,headResponseDegrees,leftFingerTravel,rightFingerDrift;public PoseReport[] poses;
    }
