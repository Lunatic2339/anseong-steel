...
{
    public static class CockpitPreviewBuilder
    {
        public const string Root = "Assets/_Project/02_Art/CockpitV01";
        ...
        [MenuItem("Anseong Steel/Cockpit/Build or Refresh V01 Preview")]
        public static void Build()
        {
            ...
            var modelPath = Root + "/Models/Anseong_Cockpit_v01.fbx";
            ...
            var root = new GameObject("Cockpit_V01");
            ...
            foreach (var side in new[] {-1,1})
            {
                ...
                pad.transform.localPosition=new Vector3(side*data.stationSpacing/2,.1f,0);
                ...
                var triangles=new int[32*12]; vertices[64]=new Vector3(0,-.1f,0); vertices[65]=new Vector3(0,.1f,0);
                for(int i=0;i<32;i++) { float a=i*Mathf.PI*2/32; vertices[i]=new Vector3(Mathf.Cos(a)*data.padRadius,-.1f,Mathf.Sin(a)*data.padRadius); vertices[i+32]=vertices[i]+Vector3.up*.2f;
            ...
            PrefabUtility.SaveAsPrefabAsset(root,Root+"/Prefabs/Cockpit_V01.prefab");
            UnityEngine.Object.DestroyImmediate(root);
            var instance=(GameObject)PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath<GameObject>(Root+"/Prefabs/Cockpit_V01.prefab"));
            ...
            var overview=AddCamera("Overview",new Vector3(0,2.64f,-5.7f),new Vector3(0,1.64f,.8f),65,cameras.transform);
            ...
            EditorSceneManager.SaveScene(scene,Root+"/Scenes/Cockpit_Preview.unity"); AssetDatabase.SaveAssets();
        ...
        static void ScheduleCapture()
        {
            ...
            capture=()=> { if(EditorApplication.isCompiling || EditorApplication.isUpdating || ++ticks<30) return; EditorApplication.update-=capture;
                try {
                    SessionState.SetBool("CockpitV01.CapturePending",false);
