...
for n in ['01_Deck','02_Station_L','03_Station_R','04_Console_L','05_Console_R','06_Arm_L','07_Arm_R','08_Ceiling_Removable','09_Hull','10_Display','11_PreviewOnly']: group(n)
...
for s,label in [(-1,'L'),(1,'R')]:
    ...
    cyl('Station_base_'+label,(x,0,.07),PAD_RADIUS+.06,.14,'Hull_Graphite',64)
    cyl('Station_alloy_'+label,(x,0,.142),PAD_RADIUS,.045,'Edge_Alloy',64)
    cyl('Station_tread_'+label,(x,0,.177),PAD_RADIUS-.04,.046,'Rubber',64)
    ring('Station_ring_'+label,(x,0,.18),PAD_RADIUS-.025,.012,'Signal_Cyan')
    for q in range(-8,9):
        yy=q*.065; half=math.sqrt(max(0,(PAD_RADIUS-.1)**2-yy**2))
        box('Tread',(x,yy,.202),(2*half,.014,.008),'Panel_Steel',.002)
    for a in [pi/4,3*pi/4,5*pi/4,7*pi/4]:
        xx=x+.7*cos(a); yy=.7*sin(a)
        o=box('Pad_lock',(xx,yy,.065),(.25,.14,.1),'Panel_Steel'); o.rotation_euler.z=a
        bolt((xx,yy,.12))
    text('Station_ID_'+label,'0'+('1' if s<0 else '2')+' / '+label,(x,-.37,.211),.105,rot=(0,0,0))
    # Inner auxiliary console below elbow level, angled toward the operator.
    current=groups['04_Console_L' if s<0 else '05_Console_R']; cx=s*.40
    box('Console_foot',(cx,.64,.09),(.36,.49,.17),'Hull_Graphite')
    box('Console_column',(cx,.70,.53),(.24,.30,.81),'Panel_Steel',.03)
    for z in [.25,.30,.35,.40]: box('Console_vent',(cx,.542,z),(.16,.012,.018),'Inset_Black',.002)
    box('Console_tray',(cx,.65,1.0),(.38,.57,.12),'Edge_Alloy',.025)
    box('Console_control',(cx,.61,1.068),(.33,.44,.024),'Inset_Black')
    o=box('Aux_screen',(cx,.91,1.20),(.34,.055,.26),'Hull_Graphite',.015); o.rotation_euler.x=math.radians(17)
    o=box('Aux_glass',(cx,.876,1.2),(.29,.016,.20),'Screen_UI',.004); o.rotation_euler.x=math.radians(17)
    text('Console_readout','SYNC / '+label,(cx,.848,1.235),.041,'Signal_Cyan')
    for row in range(3):
        for col in range(3): cyl('Aux_key',(cx+(col-1)*.079,.49+row*.075,1.092),.022,.016,'Signal_Cyan' if col==0 else 'Warning_Ochre',12)
    beam('Aux_stick',(cx,.78,1.08),(cx,.78,1.21),.022,'Rubber')
    # Outer mechanical arm. No central bridge; static fit-study only.
    current=groups['06_Arm_L' if s<0 else '07_Arm_R']; ox=s*2.20
    box('Outer_anchor',(ox,-.22,.15),(.45,.6,.30),'Hull_Graphite',.04)
    box('Outer_upright',(ox,-.22,.73),(.20,.23,1.12),'Panel_Steel')
    box('Ram_outer',(ox-s*.10,-.28,.83),(.12,.13,.66),'Inset_Black')
    beam('Ram_piston',(ox-s*.12,-.28,.87),(ox-s*.12,-.28,1.40),.028,'Edge_Alloy')
    a=(ox,-.22,1.39); b=(s*1.93,.13,1.49); c=(s*1.71,.34,1.29)
    beam('Shoulder_link',a,b,.10,'Panel_Steel'); beam('Forearm_link',b,c,.075,'Edge_Alloy')
    for pos in [a,b,c]:
        o=cyl('Arm_pivot',pos,.12,.13,'Hull_Graphite',24); o.rotation_euler.x=pi/2
        o=cyl('Pivot_cap',(pos[0],pos[1]-.075,pos[2]),.071,.023,'Warning_Ochre',24); o.rotation_euler.x=pi/2
    beam('Controller_visual_mount',(c[0],c[1],1.24),(c[0],c[1],1.41),.031,'Rubber')
    tube('Arm_hydraulic',[(ox,-.38,.4),(ox+s*.16,-.39,1.27),(s*1.98,-.03,1.64),(s*1.70,.31,1.36)],.022)
    text('Arm_tag','PORT / LEFT ARM' if s<0 else 'STBD / RIGHT ARM',(ox,-.347,.85),.049)
    # Ceiling dock hangs behind head and clear of central sightline.
    current=groups['08_Ceiling_Removable']
    box('Suspension_rail',(x,-.51,3.32),(.60,1.18,.24),'Hull_Graphite',.05)
    box('Suspension_carriage',(x,-.46,3.13),(.43,.44,.22),'Panel_Steel',.025)
    beam('Dock_spine',(x,-.58,3.05),(x,-.65,2.20),.095,'Edge_Alloy')
    box('Back_harness',(x,-.64,2.07),(.5,.22,.35),'Hull_Graphite',.06)
    for dx in [-.22,.22]:
        box('Harness_cushion',(x+dx,-.50,2.07),(.11,.12,.29),'Rubber',.035)
    for k in range(4):
        dx=(k-1.5)*.11
        tube('Suspension_cable',[(x+dx,-1.39,3.3),(x+dx,-1.12,2.75-k*.09),(x+dx,-.34,2.62-k*.07),(x+dx,-.25,3.10)],.020 if k%2 else .027)
    tube('Harness_umbilical',[(x+s*.22,-.5,3.17),(x+s*.43,-.62,2.75),(x+s*.32,-.65,2.15)],.034)
...
for station_group in ['02_Station_L','03_Station_R']:
    for ob in groups[station_group].objects: ob.location.z += PAD_TOP-.20
...
cams=[camera('Overview',(0,-5.7,2.64),(0,.8,1.64),24),camera('LeftEye',(-SPACING/2,0,PAD_TOP+EYE_HEIGHT),(-.46,3,1.73),18),camera('RightEye',(SPACING/2,0,PAD_TOP+EYE_HEIGHT),(.46,3,1.73),18),camera('LeftEye_Companion',(-SPACING/2,0,PAD_TOP+EYE_HEIGHT),(1.05,.16,1.55),20)]
...
scene['Design_notes']='v01 fit study. Spacing 2.10m; pad diameter 1.30m; pad top 0.20m; eye above pad 1.65m. Static arm proxies, no IK. Ceiling removable. Display media separate.'
...
bpy.ops.wm.save_as_mainfile(filepath=os.path.join(OUT,'Anseong_Cockpit_v01.blend'))
for cam in cams:
