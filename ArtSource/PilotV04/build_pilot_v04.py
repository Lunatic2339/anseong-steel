...
for side,s in [('Left',1),('Right',-1)]:
    ...
    tube(side+'_wrist_cuff',p(.683,0,1.445),p(.715,0,1.445),.04,.039,3,side+'ForearmTwist')
...
for side in ['Left','Right']:
    for helper,target,influence in [('UpperArmTwist','LowerArm',.35),('ForearmTwist','Hand',.9),('ShoulderArmor','UpperArm',1.0)]:
...
def export():
    ...
    bpy.ops.export_scene.fbx(filepath=str(O/'AS_Pilot_v04.fbx'),use_selection=True,object_types={'ARMATURE','MESH'},add_leaf_bones=False,bake_anim=False,axis_forward='-Z',axis_up='Y',apply_unit_scale=True,use_mesh_modifiers=True,mesh_smooth_type='FACE',path_mode='AUTO')
export()
...
bpy.context.view_layer.objects.active=rig
bpy.ops.export_scene.fbx(filepath=str(O/'AS_Pilot_v04_QuestCandidate.fbx'),use_selection=True,object_types={'ARMATURE','MESH'},add_leaf_bones=False,bake_anim=False,axis_forward='-Z',axis_up='Y',apply_unit_scale=True,use_mesh_modifiers=True,mesh_smooth_type='FACE')
...
bpy.ops.wm.save_as_mainfile(filepath=str(O/'AS_Pilot_v04.blend'))
...
bpy.ops.import_scene.fbx(filepath=str(O/'AS_Pilot_v04.fbx'))
rr={'armatures':[],'meshes':[]}
